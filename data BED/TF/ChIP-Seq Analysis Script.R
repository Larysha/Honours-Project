...
peakAnnoList <- lapply(listfiles, annotatePeak, TxDb=txdb,tssRegion=c(-100, 5000), level = "gene", verbose=FALSE)
...
FinalTFs = c('FinalTFBreast', 'FinalTFCardio', 'FinalTFKidney', 'FinalTFProstate')
...
breast_peak <- readPeakFile(filt_annot_files_list[["breastFilt"]])
cardio_peak <- readPeakFile(filt_annot_files_list[["cardioFilt"]])
kidney_peak <- readPeakFile(filt_annot_files_list[["kidneyFilt"]]) 
prostate_peak <- readPeakFile(filt_annot_files_list[["prostateFilt"]])

  
###### use the MACS score to find the peak locations in PXDN and visualize this for each tissue
covplot(breast_peak, weightCol="MACSqValue", title = "ChIP-Seq leak locations for breast tissue \naccording to binding significance")
covplot(cardio_peak, weightCol= "MACSqValue",title = "ChIP-Seq peak locations for cardiovascular tissue \naccording to binding significance")
covplot(kidney_peak, weightCol= "MACSqValue", title = "ChIP-Seq peak locations for kidney tissue \naccording to binding significance")
covplot(prostate_peak, weightCol= "MACSqValue", title = "ChIP-Seq peak locations for prostate tissue \naccording to binding significance")
...
FOXA1Breast <- filter(FinalTFBreast, TF == "FOXA1")
...
HIF1ABreast <- filter(FinalTFBreast, TF == "HIF1A")
